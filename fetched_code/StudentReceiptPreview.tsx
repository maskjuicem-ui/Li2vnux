...
export default function StudentReceiptPreview({ receipt }: StudentReceiptPreviewProps) {
  ...
  return (
    ...
    >
      <div className="bg-gradient-to-br from-blue-600 via-blue-700 to-blue-800 text-white px-12 py-10">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-6">
            ...
            <div>
              <h1 className="text-3xl font-bold mb-2 tracking-tight leading-tight">{receipt.school_name.toUpperCase()}</h1>
              <p className="text-blue-100 text-base leading-relaxed">{receipt.school_address}</p>
              <p className="text-blue-100 text-base leading-relaxed">
                {receipt.school_city}, {receipt.school_state} {receipt.school_postal_code}
              </p>
              {receipt.school_phone && (
                <p className="text-blue-100 text-sm mt-2">Phone: {receipt.school_phone}</p>
              )}
              {receipt.school_website && (
                <p className="text-blue-100 text-sm">{receipt.school_website}</p>
              )}
            </div>
          </div>
      ...
      <div className="bg-gradient-to-r from-slate-800 via-slate-900 to-slate-800 text-white px-10 py-5">
        <h2 className="text-xl font-bold uppercase tracking-[0.15em] text-center">Tuition Receipt</h2>
      </div>
        ...
        <div className="px-10 py-8 border-r border-slate-200 bg-gradient-to-br from-slate-50 to-white">
          <div className="flex items-center gap-3 mb-4">
            <div className="bg-blue-100 p-2 rounded-lg">
              <GraduationCap className="w-5 h-5 text-blue-700" strokeWidth={2} />
            </div>
            <h3 className="text-sm font-bold text-slate-700 uppercase tracking-wider">Student Information</h3>
          </div>
          <div className="space-y-3">
            <div>
              <p className="text-xs text-slate-500 font-semibold uppercase tracking-wider mb-1">Student Name</p>
              <p className="text-base font-bold text-slate-900">{receipt.student_name}</p>
            </div>
            <div>
              <p className="text-xs text-slate-500 font-semibold uppercase tracking-wider mb-1">Student ID</p>
              <p className="text-base font-semibold text-slate-700 font-mono">{receipt.student_id}</p>
            </div>
            {receipt.student_email && (
              <div>
                <p className="text-xs text-slate-500 font-semibold uppercase tracking-wider mb-1">Email</p>
                <p className="text-sm text-slate-600">{receipt.student_email}</p>
              </div>
            )}
            {receipt.student_phone && (
              <div>
                <p className="text-xs text-slate-500 font-semibold uppercase tracking-wider mb-1">Phone</p>
                <p className="text-sm text-slate-600">{receipt.student_phone}</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="px-10 py-8 bg-white">
          <div className="flex items-center gap-3 mb-4">
            <div className="bg-green-100 p-2 rounded-lg">
              <Calendar className="w-5 h-5 text-green-700" strokeWidth={2} />
            </div>
            <h3 className="text-sm font-bold text-slate-700 uppercase tracking-wider">Receipt Details</h3>
          </div>
          <div className="space-y-3">
            <div>
              <p className="text-xs text-slate-500 font-semibold uppercase tracking-wider mb-1">Date Issued</p>
              <p className="text-base font-semibold text-slate-900">{formatDate(receipt.date_issued)}</p>
            </div>
            <div>
              <p className="text-xs text-slate-500 font-semibold uppercase tracking-wider mb-1">Term</p>
              <p className="text-base font-semibold text-slate-700">{receipt.term}</p>
            </div>
            {receipt.academic_year && (
              <div>
                <p className="text-xs text-slate-500 font-semibold uppercase tracking-wider mb-1">Academic Year</p>
                <p className="text-base font-semibold text-slate-700">{receipt.academic_year}</p>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="bg-gradient-to-r from-slate-800 to-slate-900 text-center px-10 py-6 border-t border-slate-700">
        <p className="text-xs text-slate-400 leading-relaxed italic">
          This is an official receipt issued by {receipt.school_name}. For questions or concerns, please contact the Cashier's Office.
        </p>
