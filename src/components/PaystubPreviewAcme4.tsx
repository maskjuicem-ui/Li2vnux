...
export default function PaystubPreviewAcme4({ data }: PaystubPreviewProps) {
  ...
  return (
    <div id="paystub-preview" className="bg-white p-12 min-h-[1100px] font-sans">
      <div className="max-w-5xl mx-auto bg-white">
        ...
        <div className="bg-blue-100 p-6 mb-8 border border-blue-200">
          <div className="flex justify-between items-start mb-4">
            <div>
              <p className="text-sm font-bold text-blue-800 mb-1">SSN: <span className="font-mono">{data.employee_ssn}</span></p>
              <p className="text-sm font-bold text-blue-800">Taxable Marital Status:</p>
              <p className="text-sm text-slate-900">Single</p>
              <p className="text-sm font-bold text-blue-800 mt-2">Exemptions/Allowances: <span className="font-normal">0</span></p>
            </div>
            <div className="text-right">
          ...
          <table className="w-full border-collapse">
            ...
            <tbody>
              ...
              <tr className="bg-blue-700 text-white">
                <td colSpan={3} className="py-4 px-4 font-bold">Gross Pay</td>
                <td className="py-4 px-4 text-right font-bold">{formatCurrency(data.gross_pay_current)}</td>
                <td className="py-4 px-4 text-right font-bold">{formatCurrency(data.gross_pay_ytd)}</td>
              </tr>
            </tbody>
          </table>
        </div>
          ...
          <table className="w-full border-collapse">
            ...
            <tbody>
              ...
              <tr className="bg-blue-700 text-white font-bold">
                ...
                <td className="py-4 px-4 text-right">{formatCurrency(data.total_deductions_current)}</td>
                <td className="py-4 px-4 text-right">{formatCurrency(data.total_deductions_ytd)}</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div className="grid grid-cols-2 gap-8">
          <div className="bg-blue-100 p-4">
            <div className="flex justify-between items-center">
              ...
              <span className="text-2xl font-bold text-blue-700">{formatCurrency(data.net_pay_current)}</span>
            </div>
          </div>
          <div className="bg-blue-700 text-white p-4">
            <div className="flex justify-between items-center">
              <span className="font-bold">Net Pay</span>
              <span className="text-2xl font-bold">{formatCurrency(data.net_pay_ytd)}</span>
            </div>
          </div>
        </div>
      </div>
